...
jQuery( "#menuui" ).menu({
 ...
 jQuery("#dragBar").height(jQuery("#myDiagram").height());
  ...
  var GradientYellow = $(go.Brush, "Linear", { 0: "LightGoldenRodYellow", 1: "#FFFF66" });  // 颜色介于0-1之间，渐变
  ...
  // define own named figures by calling the static function Shape.defineFigureGenerator.(自定义图形形状)
  go.Shape.defineFigureGenerator("Empty", function(shape, w, h) {    // 定义一个Figure，名字为"Empty"
   // function(shape, w, h):shape -->生成的集合形状, w,h -->width & height
...
go.Shape.defineFigureGenerator("endEvent", function(shape, w, h) {
  ...
  go.Shape.defineFigureGenerator("Annotation", function(shape, w, h) {
    ...
    geo.scale(scale, scale);   // 返回改变后的Geometry
  ...
  function nodeActivityTaskTypeConverter(s) {
    var tasks = ["Empty",
        // BpmnTaskMessage,BpmnTaskUser,BpmnTaskScript都是GoJS预定义图形
                  "BpmnTaskMessage",
                  "BpmnTaskUser",
                  "BpmnTaskScript",
  ...
  function nodeEventTypeConverter(s) {  // order here from BPMN 2.0 poster
    var tasks = [ "NotAllowed",
                  "startEvent",
                  "BpmnTaskService",
        // NotAllowed 以及以下这些图形都是GoJS的预定义图形。
                  "BpmnEventTimer",
                  "Empty",
                  "BpmnTaskMessage",
  ...
  // Spot:点，地点。Spot(x,y,offx,offy)      Spot(0,0,0,0)<-->the top-left corner
 // 边界事件在结点上的放置位置
  function nodeActivityBESpotConverter(s) {     // 计算在节点上添加边界事件的位置
  ...
  // 将diagram图形中的go.Point（坐标点，例如(1,2)）location转换成浮点数组loc传给model数据
  ...
  // 将model数据的浮点数组loc转换成diagram图形中的go.Point类型location坐标点
  ...
  //--------------------------------Activity nodes template-----------------------------------
    var taskNodeTemplate =
        $(go.Node, "Spot",    // Node(type)：构造一个空的Node结点。type：controls how the Panel's elements are measured and arranged.
            {   locationObjectName: "SHAPE",
                locationSpot: go.Spot.Center,   // the Node.location is at the center of each node
                resizable: true,    // 点击结点时，结点上会显示八个handles(用于调整结点的大小).
                resizeObjectName: "PANEL",   // resize the panel,not the node.设置panel的desiredSize
    ...
    function loadServicesData() {
        ...
        jQuery.getJSON("services.json", function(json){
            jQuery.each(json.msg, function(i,sv) {
                var service = sv.sName;
                var sId = sv.sId;
                serviceMenu.add(
                    $("ContextMenuButton",
  ...
  var eventNodeTemplate =
        ...
        $(go.Panel, "Spot",
          ...
          $(go.Shape, "NotAllowed",
              { alignment: go.Spot.Center, desiredSize: new go.Size(EventNodeSymbolSize, EventNodeSymbolSize), stroke: "black" },
                // nodeEventTypeConverter函数的参数是eventType，函数返回值为事件的形状。根据事件类型（eventType）决定事件的形状
                new go.Binding("figure", "gType", nodeEventTypeConverter),
                // 根据nodeEventDimensionSymbolFillConverter函数返回值决定填充颜色
    ...
    var eventNodeTemplateForTemplate =
        $(go.Node, "Vertical",
            {   locationObjectName: "SHAPE",
                locationSpot: go.Spot.Center,
                selectionAdorned: false
  ...
  function nodeGatewaySymbolTypeConverter(s) {
    var tasks =  ["NotAllowed",
                  ...
                  "Pentagon",       // 5 - double cicle event based gateway   五边形
  ...
  var gatewayNodeTemplateForPalette =
    $(go.Node, "Vertical",
      ...
      $(go.Panel, "Spot",
         // Use the Shape class to paint a geometrical figure(使用Shape类来画几何图形)
        $(go.Shape, "Diamond",
    ...
    var lightNodeTemplateForPalette =
        $(go.Node, "Spot",
            ...
            $(go.TextBlock,
                { alignment: new go.Spot(0.5,0.5), textAlign: "center", margin: 2 },
                new go.Binding("text","name"))
        );

    //---------------------------- Node for empty category -------------------------------
  ...
  palNodeTemplateMap.add("event", eventNodeTemplateForTemplate);
        ...
        $(go.Link,
            {
                ...
                resegmentable: true,  // add/remove segments from links.点击连线时连线上显示的小菱形
    ...
    $(go.Link,
        {
            ...
            curve: go.Link.JumpOver,
            corner: 5,
            relinkableFrom: true, // 已经存在的连线的端点可以改变（默认值为false，此时连线的起始结点不能改变，结束结点可以改变）
            relinkableTo: true,   // 默认值为false，此时，连线的起始结点可以改变，结束结点不可变
            reshapable: true,
            resegmentable: true,
            toEndSegmentLength: 20,
            // mouse-overs subtly highlight links:
    ...
    $(go.Diagram, "myDiagram",
      {
        ...
        // CommandHandler对象为Diagram添加一些键盘命令，如点击Delete键删除元素，Ctrl+C复制，Ctrl+V粘贴，Ctrl+Z撤销等
        ...
        { nodeTemplateMap: palNodeTemplateMap,
          ...
          layout: $(go.GridLayout,
                    {cellSize: new go.Size(1, 1),
                     ...
                     wrappingColumn: 1,       // 始终在一列上显示图标，不换列
                     comparer: keyCompare      // comparer: Gets or sets the comparison function used to sort the parts.（规定排序方式）
                    })
        }
      );
  ...
  myPaletteLevel1.model = $(go.GraphLinksModel,
    {
      nodeDataArray: [
        { key: createUUID(), nType: "start", category: "event", name: "Start", desc: "", nConfig: {},gType: 1},
        { key: createUUID(), nType: "end", category: "event", name: "End", desc: "", nConfig: {}, gType: 8},
        { key: createUUID(), nType: "event", category: "event", name: "Timer", desc: "", gType: 3,
          nConfig: {
        ...
        { key: createUUID(), nType: "event", category: "event", name: "Device", desc: "", gType: 4,
          nConfig: {
       ...
       { key: createUUID(), nType: "activity", category: "p-service", name: "Service", desc: "", gType:0,
         nConfig:{
       ...
       { key: createUUID(), nType: "activity", category: "model", name: "Model", desc: "", gType:0,
         nConfig:{
                 aId: "", aName: "", aDesc: "", type: "task", behavior: "immediate", userId: "",
                 config: {
                     tType: "ModelTask",
       ...
       { key: createUUID(), nType: "activity", category: "r-application", name: "Application", desc: "", gType: 0,
         nConfig: {
              aId: "", aName: "", aDesc: "", type: "task", behavior: "immediate", userId: "",
              config: {
                  tType: "ApplicationTask",
                  taskArgs: { applicationId: "", cmd: ""}
              }
         }
       },

       { key: createUUID(), nType: "activity", category: "resultAnalysis", name: "resultAnalysis", desc: "", gType: 0,
         nConfig:{
                 aId: "", aName: "", aDesc: "", type: "task", behavior: "immediate", userId: "",
                 config: {
                     tType: "AnalysisTask",
       ...
       { key: createUUID(), nType: "activity", category: "task", name: "deviceTask", desc: "", gType: 4,
          nConfig: {
        ...
        { key: createUUID(), nType: "activity", category: "task", name: "scriptTask", desc: "", gType: 3,
          nConfig: {
                  aId: "", aName: "", aDesc: "", type: "task", userId: "",
                  behavior: "immediate", behaviorConfig: {exeTimes: 4, cron: "*/20 * * * * ?"},
                  config: {
        ...
        { key: createUUID(), nType: "activity", category: "task", name: "userTask", desc: "", gType: 2,
          nConfig: {
                  aId: "", aName: "", aDesc: "", type: "task", userId: "",
                  behavior: "immediate", behaviorConfig: { exeTimes: 4, cron: "*/20 * * * * ?"},
                  config: {
        ...
        { key: createUUID(), nType: "activity", category: "s-subprocess", name: "Subprocess", desc: "",
        ...
        { key: createUUID(), nType: "activity", category: "s-callactivity", name: "callActivity", desc: "",
          isGroup: true, isSubProcess: true, gType: 0,
          nConfig: {
        ...
        { key: createUUID(), nType: "gateway", category: "zgateway", name: "Parallel", desc: "", gType: 1,
          nConfig: { type: "parallel" }
        },
        { key: createUUID(), nType: "gateway", category: "zgateway", name: "Exclusive", desc: "", gType: 4,
          nConfig: { type: "exclusive" }
        },
        { key: createUUID(), nType: "gateway", category: "zgateway", name: "Event", desc: "", gType: 5,
          nConfig: { type: "event" }
        },
        { key: createUUID(), nType: "gateway", category: "zgateway", name: "Inclusive", desc: "", gType: 2,
          nConfig: { type: "inclusive" }
        },
        { key: createUUID(), nType: "gateway", category: "zgateway", name: "ExclusiveEvent", desc: "", gType: 6,
          nConfig: { type: "exclusiveEvent" }
        },
        { key: createUUID(), nType: "gateway", category: "zgateway", name: "ParallelEvent", desc: "", gType: 7,
...
function addBoundaryEvent(evType) {
    ...
    myDiagram.selection.each(function(node) {
            ...
            if(node.data && (node.data.category === "task")) {
                var i = 0;
                var defaultPort = node.findPort("");       // defaultPort返回的是整个节点对象
...
function addLinkConfiguration(obj){
    ...
    if(obj.fromNode.data.gType !== 4){
        ...
        // 因为初始时每条link的condition字段都未定义，所以页面显示undefined，通过if条件判断将初始的undefined改为“${}”
...
// save the task node's configuration
...
function saveApplicationConfig(id){
    myDiagram.startTransaction("saveApplicationConfig");
    ...
    myDiagram.commitTransaction("saveApplicationConfig");
...
function saveProConfiguration(saveReturnId) {
    ...
    // 数据存在，但使用myDiagram.model.toJson()不显示
   // alert(myDiagram.model.proName);
